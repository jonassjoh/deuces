    ...
    def hand_summary(self, board, hands):
        ...
        for i in range(len(stages)):
            ...
            for player, hand in enumerate(hands):
                ...
                print( "Player %d hand = %s, percentage rank among all hands = %f" % (player + 1, class_string, percentage) )
            ...
            # otherwise on all other streets
                ...
                if len(winners) == 1:
                    print( "Player %d is the winner with a %s\n" % (winners[0] + 1, self.class_to_string(self.get_rank_class(self.evaluate(hands[winners[0]], board)))) )
                else:
